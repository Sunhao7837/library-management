...
<!--    搜索表单-->
    ...
    <el-table :data="tableData" stripe row-key="id" default-expand-all fit>
...
export default {
  ...
  methods: {
    load() {
      request.get('/borrow/pageRestitution', {
        params: this.params
      }).then(res => {
        if(res.code === '200') {
    ...
    del(id) {
      request.delete("/borrow/deleteRestitution/" + id).then(res => {
        if (res.code === '200') {
